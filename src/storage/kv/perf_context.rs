...
pub fn persist_perf_data(write_perf: &mut LocalHistogramVec, db: &'static str, delta: PerfStatisticsDelta) {
    write_perf
    ...
    // write_perf.with_label_values(&[db, "key_lock_wait_time"]).observe(delta.key_lock_wait_time as f64);
    // write_perf.with_label_values(&[db, "key_lock_wait_count"]).observe(delta.key_lock_wait_count as f64);
}
